...
const DonorApproval = () => {
  ...
  const refreshSidebarContent = async () => {
    const freshSubmissions = await fetchSubmissions();
    if (selectedSubmission) {
      const updatedSubmission = freshSubmissions.find(
        (s: Submission) => s.id === selectedSubmission.id
      );
  ...
  const generateDonorId = () => {
    // Generate a unique donor ID: D-YYYYMMDD-XXXXX (where X is random alphanumeric)
    const now = new Date();
  ...
  const handleApprove = async () => {
    ...
    setProcessing(true);
    try {
      // Create donor record from submission with a unique donor_id
            ...
            <>
              ...
              <div className="mt-6 space-y-6">
                ...
                <EvaluationDetails
                  ...
                  onEvaluationComplete={refreshSidebarContent}
                />
